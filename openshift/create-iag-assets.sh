...
oc delete secret iag > /dev/null 2>&1
echo "Creating new iag Secret from common/secret_files"
oc create secret generic iag --from-file=${RUNDIR}/common/secret_files/
echo "Deleting iag ConfigMap (if it exists)"
oc delete configmap iag > /dev/null 2>&1
echo "Creating new iag ConfigMap from common/env_files"
oc create configmap iag --from-file=${RUNDIR}/common/env_files/
    ...
    if [[ ! ${line} =~ ^[[:space:]]*#.* ]]; then
      if [[ ${line} =~ ^.*=.* ]]; then
        ...
        if [[ ${key} == S_* ]]; then
          ...
          oc patch secret iag -p "{\"data\":{\"${key}\":\"${b64value}\"}}"
        else
          echo "Adding ${key} to ConfigMap"
          oc patch configmap iag -p "{\"data\":{\"${key}\":\"${value}\"}}"
        fi
      fi
    fi
